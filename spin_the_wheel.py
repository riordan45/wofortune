...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,guessed):
        ...
        if self.prizeMoney >= VOWEL_COST:
            ...
            for a in LETTERS:
                if not (a in guessed) and (not(a in VOWELS)):
                    choices.append(a)
        return choices
    
    def getMove(self,category, obscuredPhrase, guessed):
        trial = self.getPossibleLetters(guessed)
        if len(trial) == 0:
            return 'pass'
        move = self.smartCoinFlip()
        if move == True:
            for a in reversed(self.SORTED_FREQUENCIES):
                if a in trial:
                    return a
        else:
            return random.choice(trial)
